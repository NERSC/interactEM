...
public:
    ...
    void push_rdma(const std::string &header, nb::ndarray<nb::numpy> data)
    {
        try
        {
            ...
            auto [serialized_bytes, ndarray_data] = serialization::CapnProtoSerializer::serialize(header, data);
            ...
            try
            {
                rdma_segments.emplace_back(reinterpret_cast<void *>(serialized_bytes.data()), serialized_bytes.size());
                std::cout << "Debug: Added Cap'n Proto message segment of size = " << serialized_bytes.size() << " bytes." << std::endl;
            }
            catch (const std::exception &e)
            {
                std::cout << "Error while adding Cap'n Proto message segment: " << e.what() << std::endl;
            }
            ...
            try
            {
                ...
                std::cout << "Debug: Added ndarray segment of size = " << data.size() << " bytes." << std::endl;
            }
            catch (const std::exception &e)
            {
                std::cout << "Error while adding ndarray segment: " << e.what() << std::endl;
            }

            if (rdma_segments.size() > std::numeric_limits<std::size_t>::max())
            {
                std::cout << "Debug: Segments size (after adding) = " << rdma_segments.size() << " exceeds maximum acceptable limit." << std::endl;
                throw std::length_error("Segments size exceeds maximum acceptable limit.");
            }

            // Create a bulk object with read-only access to the data
            std::cout << "Debug: Creating a bulk object for data exposure." << std::endl;
